...
module.exports = function(metadata = {}, src, dest = '.'){
  ...
  return new Promise((resolve, reject) => {
    ...
    metalsmith.use((files, metalsmith, done) => {
      ...
      if (meta.test === 'None') {
        Object.keys(files).forEach(fileName => {
          if (/test|spec|\_\_tests\_\_/.test(fileName)) {
            delete files[fileName]
          }
        })
        delete files['file-transformer.js']
        delete files['jest-css-modules-transform-config.js']
        delete files['jest.config.js']
        delete files['karma.conf.js']
      }
      if (meta.test === 'Jest') {
        Object.keys(files).forEach(fileName => {
    ...
    metalsmith.use((files, metalsmith, done) => {
      ...
      if (meta.test === 'None') {
        ...
        delete devDependencies['chai-enzyme']
        delete devDependencies['enzyme']
        delete devDependencies['enzyme-adapter-react-16']
        delete devDependencies['jest']
        delete devDependencies['jest-css-modules-transform']
        delete devDependencies['karma']
        delete devDependencies['karma-chai']
        delete devDependencies['karma-chrome-launcher']
        delete devDependencies['karma-mocha']
        delete devDependencies['karma-webpack']
        delete devDependencies['mocha']
        delete devDependencies['ts-jest']
        delete devDependencies['@types/chai']
        delete devDependencies['@types/chai-enzyme']
        delete devDependencies['@types/enzyme']
        delete devDependencies['@types/enzyme-adapter-react-16']
        delete devDependencies['@types/jest']
        delete devDependencies['@types/mocha']
      }
      if (meta.test === 'Jest') {
        delete devDependencies['chai']
        delete devDependencies['chai-enzyme']
      ...
      if (meta.test === 'Mocha') {
        ...
        delete devDependencies['ts-jest']
        delete devDependencies['@types/jest']
        pkg.scripts['test'] = pkg.scripts['test:karma']
      }
      // if (!meta.stylus) {
      ...
      delete pkg.scripts['test:karma']
      delete pkg.scripts['test:jest']
